...
const CompSvg = ({currentData}) => {
  ...
  const fnGetDashoffset = function(){
    let ratio
    let distance
    let display = 'block'
    if(sunrise&&sunset){
      ratio = 1 - (dt - sunrise)/ (sunset - sunrise)
      distance = (dt - sunrise)/ (sunset - sunrise) * 100
      if(ratio<0) ratio = 0
      if(ratio>1) ratio = 1
    }
    else{
  ...
  const {ratio,distance,display} = fnGetDashoffset()
  return (
    <svg viewBox="0 0 400 200">
      <path className="line" d="M25,200c0-96.65,78.35-175,175-175s175,78.35,175,175" />
      <path style={{strokeDashoffset:`${ratio}`,display:`${display}`}} pathLength={1} className="dash" d="M25,200c0-96.65,78.35-175,175-175s175,78.35,175,175" />
      <path style={{offsetDistance:`${distance}%`,display:`${display}`}} className="sun" d="M0-18.77c-10.37,0-18.77,8.41-18.77,18.77S-10.37,18.77,0,18.77S18.77,10.37,18.77,0S10.37-18.77,0-18.77z
        ...
        <defs>
          <linearGradient id="gradient" x1="0%" y1="0%">
            ...
            <stop offset="34%" stopColor="#f98a2c" />
            <stop offset="58%" stopColor="#ed5b45" />
            <stop offset="76%" stopColor="#b64262" />
            <stop offset="92%" stopColor="#633663" />
            <stop offset="100%" stopColor="#402852" />
          </linearGradient>
        </defs>
    </svg>
  );
